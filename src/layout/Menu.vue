<script setup lang="ts">
import { ref } from "vue";

export interface IMeunu {
  path: string;
  name: string;
  icon: string;
}

const routes = ref<IMeunu[]>([
  {
    path: "/",
    name: "Home",
    icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6",
  },
  {
    path: "staff",
    name: "Staff",
    icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4",
  },
  {
    path: "records",
    name: "Records",
    icon: "M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4",
  },
  {
    path: "talks",
    name: "Talks",
    icon: "M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z",
  },
  {
    path: "settings",
    name: "Settings",
    icon: "M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4",
  },
]);
</script>

<template>
  <div class="bg-white w-[270px] h-screen border-1 sumir">
    <div class="mt-10">
      <router-link
        :to="route.path"
        v-for="route in routes"
        :key="route.name"
        class="flex items-center px-3 py-2 mx-10 my-3 hover:text-white hover:bg-indigo-500 hover:rounded-xl"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-6 h-6"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          stroke-width="2"
        >
          <path stroke-linecap="round" stroke-linejoin="round" :d="route.icon" />
        </svg>
        <p class="pl-3 font-semibold" exact-active-class="exact-active">
          {{ route.name }}
        </p>
      </router-link>
    </div>
  </div>
</template>

<style scoped>
@media only screen and (max-width: 700px) {
  .sumir {
    display: none;
  }
}
</style>